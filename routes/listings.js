...
router.get("/add",isLoggedIn,wrapAsync(async(req,res,next)=>{
...
router.get("/:title", wrapAsync(async (req, res,next) => {
    titleError(req,next);
      ...
      const listing = await Listing.findOne({ title: title }).populate({path:"reviews",populate:{path:"user"}}).populate("owner");
      if(!listing){
      ...
      res.render("listings/show.ejs", { listing: listing,geoapifyKey: process.env.GEOAPIFY_API_KEY });
  }));
  router.delete("/:title/delete",isLoggedIn,isOwner, wrapAsync(async (req, res,next) => {
    titleError(req,next);
  ...
  router.put("/:title/edit",isLoggedIn,isOwner,upload.single("image"),geocode, wrapAsync(async (req, res,next) => {
